...
import 'package:inspector/inspector/keyboard_handler.dart';
import 'package:inspector/inspector/widgets/zoom/zoom_overlay.dart';
import 'package:inspector/mobx/inspector_controller/inspector_controller.dart';
...
class InspectorState extends State<Inspector> {
  ...
  final _inspectorPositionNotifier =
      ValueNotifier<Offset?>(const Offset(0, 20));
  ...
  Widget build(BuildContext context) {
    ...
    return Provider<InspectorController>(
      create: (BuildContext context) => InspectorController()..loadPosition(),
      child: Stack(
        key: _stackKey,
        children: [
          Align(
          ...
          if (_isPanelVisible)
            MultiValueListenableBuilder(
              valueListenables: [
                ...
                _zoomStateNotifier,
                _byteDataStateNotifier,
                _inspectorPositionNotifier
              ],
              builder: (context) => Observer(
                builder: (context) {
                  final inspectorController =
                      context.watch<InspectorController>();
                  // if (inspectorController.inspectorOffset.isLoading) {
                  // }
                  if (inspectorController.inspectorOffset.isValue) {
                    return Positioned(
                      left: inspectorController.inspectorOffset.value!.dx,
                      top: inspectorController.inspectorOffset.value!.dy,
                      child: Draggable(
                        onDragEnd: (details) {
                          inspectorController.writePosition(details.offset);
                        },
                        childWhenDragging: const SizedBox(),
                        feedback: InspectorPanel(
                          isTestStagesEnabled: widget.isTestStagesEnabled!,
                          ...
                          testStagesCallback: widget.testStagesCallback,
                          isInfospecEnabled: true,
                        ),
